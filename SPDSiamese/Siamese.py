...
def train_siamese(epoch, train_loader, model, loss_fn, optimizer, cuda, N, tot, tot_label, rep, m):
    ...
    for batch_idx, x in bar:
        ...
        if batch_idx == len(train_loader) - 1:
            ...
            Acc = d[d == 0].size(0)
            bar.set_description('Loss: %.3f|Acc : %.2f%% (%d/%d)' % (total_loss / (batch_idx + 1), 100. * Acc / len(x), Acc, len(x)))
...
def test_siamese(epoch, dataloader_val, model, loss_fn, cuda, N, tot, tot_label, rep, m):
    ...
    for batch_idx, x in bar:
        ...
        total += target.size(0)
        bar.set_description('Loss: %.3f |(%d/%d)' % (test_loss / (batch_idx + 1.0), 0, total)) 
        if batch_idx == len(dataloader_val) - 1:
            ...
            d = dist_matrix.min(1)[1] - tot_label
            Acc = d[d == 0].size(0)
            bar.set_description('Loss: %.3f|Acc : %.2f%% (%d/%d)' % (test_loss / (batch_idx + 1), 100. * Acc / len(x), Acc, len(x)))
    ...
    def __init__(self, Data, N):
        ...
        if self.train:
            ...
            self.label_to_indices = {i: torch.where(self.target == i)[0] for i in range(N) 
                                    if len(torch.where(self.target == i)[0]) >= 10}
            x = self.label_to_indices
            pos_pairs = [[x[m][i].item(), x[m][i + 1].item(), 1] for m in x for i in range(-1, len(x[m]) - 1)
                         ] + [[x[m][i].item(), x[m][i - 3].item(), 1] for m in x for i in range(3, len(x[m]) - 1, 3)]
            t = []
            Mm = len(pos_pairs)
            h = Mm // len(x) + 1
            for m in x:
                r1 = torch.full((h, ), m).tolist()
                r2 = random.sample([i for j in range(h // (len(x) - 1) + 1) for i in set(x.keys()) - {m}], h)
                r3 = torch.arange(len(x[m])).tolist() 
                if len(r3) < h:
                    r3 += [random.randint(0, len(x[m]) - 1) for i in range(h - len(x[m]))]
                    r3 = random.sample(r3, h)
                else:
                    r3 = random.sample(r3, h)
